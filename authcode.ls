  ...
  { get-auth-code-url, try-get-token-response-for-auth-code } = dependency 'google.OAuth'
  ...
  errorlevel++ ; { error, value: token-response } = try-get-token-response-for-auth-code client-id, client-secret, auth-code
  if error isnt void => stderr-lines [ "Unable to get token response for auth code.", error.message ]
